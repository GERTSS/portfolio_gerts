...
DATA_AUTHORS = [
    {"id": 1, "first_name": "Владимир", "last_name": "Смоляр", "middle_name": ""},
...
def get_book(id):
    with sqlite3.connect(DATABASE_NAME) as conn:
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM `{BOOKS_TABLE_NAME}` WHERE id = ?", (id,))
        book = cursor.fetchall()
        return [_get_book_obj_from_row(row) for row in book]
...
def delete_book(id):
    with sqlite3.connect(DATABASE_NAME) as conn:
        ...
        cursor.execute(f"SELECT * FROM `{BOOKS_TABLE_NAME}` WHERE id = ?", (id,))
        book = cursor.fetchall()
        cursor.execute(f"DELETE FROM `{BOOKS_TABLE_NAME}` WHERE id = ?", (id,))
        return [_get_book_obj_from_row(row) for row in book]
...
def get_all_books_by_author(author_id):
    with sqlite3.connect(DATABASE_NAME) as conn:
        cursor = conn.cursor()
        cursor.execute(
            f"SELECT * FROM `{BOOKS_TABLE_NAME}` WHERE author = ?", (author_id,)
        )
        books = cursor.fetchall()
        return [_get_book_obj_from_row(row) for row in books]


def delete_books_by_id_author(author_id: int) -> None:
    with sqlite3.connect(DATABASE_NAME) as conn:
        cursor = conn.cursor()
